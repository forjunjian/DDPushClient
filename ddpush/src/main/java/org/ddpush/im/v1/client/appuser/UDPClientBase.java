...
public abstract class UDPClientBase implements Runnable {
	...
	private void ackServer(Message m) throws Exception {
		...
		if ( m.getCmd() == Message.CMD_0x11 ) {
			...
			byte[] data   = m.getData();
			ByteBuffer.wrap( buffer ).put( ( byte ) Message.version ).put( ( byte ) appid ).put( ( byte ) Message.CMD_0x11 ).put( uuid ).putChar( ( char ) 8 ).put( data, Message
				.SERVER_MESSAGE_MIN_LENGTH, 8 );
			send( buffer );
		}
